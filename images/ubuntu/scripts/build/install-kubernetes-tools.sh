...
# Download KIND
kind_url=$(resolve_github_release_asset_url "kubernetes-sigs/kind" "endswith(\"kind-linux-$(get_arch "amd64" "arm64")\")" "latest")
kind_binary_path=$(download_with_retry "${kind_url}")

# Supply chain security - KIND
kind_external_hash=$(get_checksum_from_url "${kind_url}.sha256sum" "kind-linux-$(get_arch "amd64" "arm64")" "SHA256")
...
# Download minikube
curl -fsSL -O https://storage.googleapis.com/minikube/releases/v1.34.0/minikube-linux-$(get_arch "amd64" "arm64")

# Supply chain security - minikube
minikube_hash=$(get_checksum_from_github_release "kubernetes/minikube" "linux-$(get_arch "amd64" "arm64")" "1.34.0" "SHA256")
use_checksum_comparison "minikube-linux-$(get_arch "amd64" "arm64")" "${minikube_hash}"

# Install minikube
install minikube-linux-$(get_arch "amd64" "arm64") /usr/local/bin/minikube
