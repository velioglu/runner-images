...
echo "deb [arch=$(get_arch "amd64" "arm64") signed-by=$GPG_KEY] $REPO_URL ${os_codename} stable" > $REPO_PATH
apt-get update
...
for plugin in $plugins; do
    ...
    filter=$(get_toolset_value ".docker.plugins[] | select(.plugin == \"$plugin\") | .asset.$(get_arch "x64" "arm64")")
...
aws_helper_url=$(curl -fsSL "${aws_latest_release_url}" | jq -r '.body' | awk -F'[()]' '/linux-'"$(get_arch "amd64" "arm64")"'/ {print $2}')
