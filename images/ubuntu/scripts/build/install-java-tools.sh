...
create_java_environment_variable() {
    ...
    local install_path_pattern="/usr/lib/jvm/temurin-${java_version}-jdk-$(get_arch "amd64" "arm64")"
    ...
    echo "Setting up JAVA_HOME_${java_version}_$(get_arch "X64" "arm64") variable to ${install_path_pattern}"
    set_etc_environment_variable "JAVA_HOME_${java_version}_$(get_arch "X64" "arm64")" "${install_path_pattern}"
}
...
install_open_jdk() {
    ...
    touch "${java_toolcache_version_path}/$(get_arch "x64" "arm64").complete"

    # Create symlink for Java
    ln -s ${java_version_path} "${java_toolcache_version_path}/$(get_arch "x64" "arm64")"
